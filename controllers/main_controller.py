...
from config import db_config
from config.constants import APP_SECRET_KEY, DEFUALT_LIMIT, DEFUALT_OFFSET

import jwt

# openapi configuartion
info = Info(title="Flask-Mongo-MVC API", version="0.0.1")
...
def generatePdf(res, pdfpath):
    try:
        ...
        nCnt, log_arr = list_logs(filterArr = {}, offset = 0, limit = 10, orderset = '_id')
        res['logs'] = log_arr
...
def get_employees_list(query: BasicQueryString, body: UserModel):
    try:
        offset = int(query['offset']) if 'offset' in query else DEFUALT_OFFSET
        limit = int(query['limit']) if 'limit' in query else DEFUALT_LIMIT
        order_by = query['order_by'] if 'order_by' in query else "_id"
        order_direction = query['order_direction'] if 'order_direction' in query else "desc"
        orderset = "-"+order_by if order_direction == "desc" else "+"+order_by
        search_term = query['search_term'] if 'search_term' in query else None
        
        filterArr = {'user_id': ObjectId(body.dict()['user_id'])}  
        ...
        # just an example
        nCnt, data = list_employees(filterArr, offset, limit, orderset)
